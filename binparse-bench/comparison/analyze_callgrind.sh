#!/usr/bin/env bash
# Deterministic binparse-vs-simple-dns DNS comparison (instruction + alloc counts).
#
# The two head-to-head programs live as examples of binparse-bench:
#   comparison/dns_binparse.rs   -> target/release/examples/dns_binparse
...
#     cargo build -p binparse-bench --examples --release
#   valgrind --tool=callgrind --cache-sim=no --branch-sim=no \
#     --callgrind-out-file=/tmp/cg_bp.out ./target/release/examples/dns_binparse 1000
#   valgrind --tool=callgrind --cache-sim=no --branch-sim=no \
...
echo
echo "===== per-work() call counts (binparse, total/1000) ====="
gawk '
...
echo
echo "===== binparse top self-Ir functions ====="
