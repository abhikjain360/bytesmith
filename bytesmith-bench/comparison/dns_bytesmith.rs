//! Hot loop: parse a DNS response with the bytesmith-generated parser and read
//! EVERY field (identical to the `dns` criterion bench's `bytesmith-full` arm) —
...
fn main() {
    ...
    black_box(acc);
    eprintln!(
        "bytesmith dns: {iters} iters in {el:?} = {:.1} ns/iter",
