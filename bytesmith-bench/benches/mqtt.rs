//! MQTT 3.1.1 + v5: bytesmith vs the rumqtt codecs.
//!
...
//! structs (allocating topic/payload), whereas bytesmith borrows the input and
...
//! `iter_batched` setup (untimed) because `read()` drains them; bytesmith borrows
...
// encode), mirroring bytesmith's `to_vec`. The model is the mirror image of the
// read benches: bytesmith's `*Content` borrows the field slices (`&'static`
...
fn v3_connect_write(c: &mut Criterion) {
    use bytesmith_protocols::mqtt_v3::{
...
fn v5_connack_write(c: &mut Criterion) {
    use bytesmith_protocols::mqtt_v5::{
