//! TLS record: bytesmith vs tls-parser (nom-based, rusticata).
//!
//! Compared against `parse_tls_raw_record`, which — like bytesmith's `TlsRecord`
...
fn tls_record(c: &mut Criterion) {
    ...
    g.bench_function("bytesmith", |b| {
        b.iter(|| {
            let (mut rec, _) =
                bytesmith_protocols::tls::TlsRecord::parse(black_box(TLS_RECORD)).unwrap();
            black_box(
