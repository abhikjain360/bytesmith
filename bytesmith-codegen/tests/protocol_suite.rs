...
mod tests {{
    ...
    fn assert_tree_snapshot(name: &str, tree: bytesmith::FieldNode<'_>) {{
    ...
    fn dns_truncated_rdata_dissect_partial_tree() {{
        ...
        assert!(matches!(rdlength.status, bytesmith::Status::Ok));
        let rdata = tree
            .children
            .iter()
            .find(|c| c.name == "rdata")
            .expect("rdata node present");
        assert!(matches!(rdata.status, bytesmith::Status::Error(_)));
    }}
    ...
    fn dissect_truncated_ipv4_yields_partial_tree() {{
        ...
        assert_eq!(tree.children[0].value, bytesmith::Value::UInt(4));
