...
fn ipv6_writer_round_trips_and_derives_payload_len() {
    // NOTE: only Ipv6 from ip.bsm produces a writer; Ipv4 produces none because of
...
fn sctp_chunk_writer_round_trips() {
    // NOTE: only SctpChunk from sctp.bsm produces a writer; Sctp produces none
...
fn dhcp_option_writer_round_trips() {
    // NOTE: only DhcpOption from dhcp.bsm produces a writer; Dhcp produces none
