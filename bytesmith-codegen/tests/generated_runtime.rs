...
fn counting_double_it(value: u16, _ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<u32> {{
    COUNTING_TRANSFORM_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    Ok(u32::from(value) * 2)
}}

fn failing_transform(_value: u16, ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<u32> {{
    Err(bytesmith::ParseError::HookFailed {{
        field: ctx.field,
...
fn counting_cstring(data: &[u8], ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<(String, usize)> {{
    COUNTING_HOOK_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    bytesmith::hooks::cstring(data, ctx)
}}

fn counting_value_cstring(data: &[u8], ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<(String, usize)> {{
    COUNTING_VALUE_HOOK_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    bytesmith::hooks::cstring(data, ctx)
}}

fn counting_len_value_cstring(data: &[u8], ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<(String, usize)> {{
    COUNTING_LEN_VALUE_HOOK_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    bytesmith::hooks::cstring(data, ctx)
}}

fn counting_union_cstring(data: &[u8], ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<(String, usize)> {{
    COUNTING_UNION_HOOK_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    bytesmith::hooks::cstring(data, ctx)
}}

fn lying_hook(data: &[u8], _ctx: bytesmith::HookContext<'_>) -> bytesmith::ParseResult<(u8, usize)> {{
    Ok((0, data.len() + 100))
}}
...
mod tests {{
    ...
    where
        F: for<'b> Fn(&'b [u8]) -> bytesmith::FieldNode<'b>,
    {{
        for len in 0..=data.len() {{
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {{
    ...
    fn dissect_truncated_validated_keeps_decoded_prefix() {{
        ...
        assert_eq!(tree.children[0].value, bytesmith::Value::UInt(0x89504e47));
        let last = tree.children.last().unwrap();
        assert_eq!(last.name, "total_len");
        assert!(matches!(last.status, bytesmith::Status::Error(_)));
        assert!(matches!(tree.status, bytesmith::Status::Error(_)));
    ...
    fn dissect_bad_magic_reports_validation_error_and_continues() {{
        ...
        assert_eq!(
            magic.status,
            bytesmith::Status::Error(bytesmith::ParseError::ValidationFailed {{
        ...
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(20));
        assert_eq!(tree.status, bytesmith::Status::Ok);
    ...
    fn dissect_unknown_variant_surfaces_failed_node() {{
        ...
        assert_eq!(body.status, bytesmith::Status::Failed("UNKNOWN_KIND"));
        assert_eq!(tree.status, bytesmith::Status::Ok);
    ...
    fn baseline_valid_packet_decodes() {{
        ...
        let fixed = packet
            .fixed()
            .unwrap()
            .collect::<bytesmith::ParseResult<Vec<_>>>()
            .unwrap();
    ...
    fn offsets_report_absolute_bit_ranges() {{
        ...
        assert_eq!(packet.n_start_offset(), bytesmith::Len::ZERO);
        ...
        assert_eq!(packet.payload_end_offset(), bytesmith::Len {{ byte: 20, bit: 0 }});
    ...
    fn cross_byte_bitfield_offsets_and_values() {{
        ...
        assert_eq!(packet.mid_start_offset(), bytesmith::Len {{ byte: 0, bit: 5 }});
        assert_eq!(packet.mid_end_offset(), bytesmith::Len {{ byte: 1, bit: 3 }});
    ...
    fn hooks_decode_and_do_not_panic_on_truncation() {{
        ...
        assert!(matches!(
            ...
            Err(bytesmith::ParseError::NotEnoughData {{ .. }})
        ));
    ...
    fn cache_value_memoizes_fixed_hook_value() {{
        ...
        assert_eq!(value_node.value, bytesmith::Value::UInt(4));
        assert_eq!(COUNTING_TRANSFORM_CALLS.load(std::sync::atomic::Ordering::Relaxed), 1);
    }}
    ...
    fn fixed_hook_ok_decodes_transformed_value() {{
        ...
        let value_node = tree.children.iter().find(|c| c.name == "value").unwrap();
        assert_eq!(value_node.value, bytesmith::Value::UInt(4));
        assert!(matches!(value_node.status, bytesmith::Status::Ok));
    }}
    ...
    fn fixed_hook_err_surfaces_in_parse_and_is_recoverable_in_dissect() {{
        ...
        assert!(matches!(
            ...
            Err(bytesmith::ParseError::HookFailed {{ field: "FailTransform.value", .. }})
        ));
        ...
        assert_eq!(suffix.value, bytesmith::Value::UInt(2));
    ...
    fn len_hook_consuming_less_than_window_exposes_rest() {{
        ...
        assert_eq!(rest.value, bytesmith::Value::Bytes(&[0xAA, 0xBB, 0xCC]));
    ...
    fn hook_in_conditional_present_path_decodes() {{
        ...
        assert_eq!(doubled.value, bytesmith::Value::UInt(10));
    }}
    ...
    fn hook_in_conditional_absent_path_skips() {{
        ...
        let tree = packet.field_tree();
        let doubled = tree.children.iter().find(|c| c.name == "doubled").unwrap();
        assert_eq!(doubled.value, bytesmith::Value::Absent);
    ...
    fn validation_failures_report_field_and_actual_value() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::ValidationFailed {{
                field: "Validated.magic",
                actual: 0x88504e47,
            }}
        );
    ...
    fn max_iter_bounds_sized_array() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::MaxIterationsExceeded {{
                field: "Capped.vals",
                max: 4,
            }}
        );
    ...
    fn padded_fields_decode_and_report_offsets() {{
        ...
        assert_eq!(packet.a_bit_range(), 0..8);
        assert_eq!(packet.b_start_offset(), bytesmith::Len {{ byte: 3, bit: 0 }});
    ...
    fn dynamic_align_errors_on_misaligned_offset() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::Misaligned {{
                field: "DynAligned.word",
                align: 2,
                offset: bytesmith::Len {{ byte: 3, bit: 0 }},
            }}
        );
    ...
    fn union_dynamic_variant_truncation_errors_instead_of_panicking() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::NotEnoughData {{ expected: 4, got: 2 }}
        );
    ...
    fn unions_in_concat_decode_independently() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::NotEnoughData {{ expected: 3, got: 2 }}
        );
    ...
    fn len_bounded_struct_ref_truncation_fails_parse() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::NotEnoughData {{ expected: 7, got: 6 }}
        );
    ...
    fn len_bounded_union_decodes_within_bound_and_exposes_rest() {{
        ...
        assert_eq!(value.value, bytesmith::Value::UnionVariant("Pair"));
        assert_eq!(value.children.last().unwrap().name, "rest");
        assert_eq!(
            value.children.last().unwrap().value,
            bytesmith::Value::Bytes(&[0xcc, 0xdd])
        );
    }}
    ...
    fn len_bounded_union_truncation_and_dissect_no_panic() {{
        ...
        assert_eq!(
            ...
            bytesmith::ParseError::NotEnoughData {{ expected: 7, got: 5 }}
        );
    ...
    fn len_bounded_greedy_array_consumes_window() {{
        ...
        assert_eq!(body_node.value, bytesmith::Value::Array);
    ...
    fn field_tree_reports_names_paths_values_and_ranges() {{
        ...
        assert_eq!(tree.value, bytesmith::Value::Struct);
        assert_eq!(tree.status, bytesmith::Status::Ok);
        let names: Vec<_> = tree.children.iter().map(|c| c.name.as_str()).collect();
        assert_eq!(
            names,
        ...
        assert_eq!(tree.children[0].value, bytesmith::Value::UInt(1));
        assert_eq!(tree.children[0].bit_range, 0..8);
        assert_eq!(tree.children[0].path, "Baseline.n");
        assert_eq!(tree.children[1].value, bytesmith::Value::UInt(0x1234));
        assert_eq!(tree.children[2].value, bytesmith::Value::UInt(0x0102_0304));
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(5));
        ...
        assert_eq!(tree.children[4].value, bytesmith::Value::UInt(13));

        let fixed = &tree.children[5];
        assert_eq!(fixed.value, bytesmith::Value::Array);
        ...
        assert_eq!(
            elems,
            [bytesmith::Value::UInt(9), bytesmith::Value::UInt(8), bytesmith::Value::UInt(7)]
        );
        ...
        assert_eq!(one.children[0].value, bytesmith::Value::UInt(0xfe));
    ...
    fn field_tree_reports_signed_values() {{
        ...
        let tree = packet.field_tree();
        assert_eq!(tree.children[0].value, bytesmith::Value::Int(-1));
        assert_eq!(tree.children[0].type_name, "i8");
        assert_eq!(tree.children[1].value, bytesmith::Value::Int(-2));
        assert_eq!(tree.children[4].value, bytesmith::Value::Int(-5));
        let vals = &tree.children[5];
        assert_eq!(vals.type_name, "[i16]");
        assert_eq!(vals.children[0].value, bytesmith::Value::Int(5));
        assert_eq!(vals.children[1].value, bytesmith::Value::Int(-5));
        let small = &tree.children[6];
        assert_eq!(small.children[0].value, bytesmith::Value::Int(127));
        assert_eq!(small.children[1].value, bytesmith::Value::Int(-128));
    }}
    ...
    fn field_tree_marks_skip_and_padding_hidden() {{
        ...
        assert_eq!(pad.value, bytesmith::Value::Bytes(&[0u8, 0]));
    }}
    ...
    fn field_tree_len_bounded_includes_rest_and_error_status() {{
        ...
        assert_eq!(value.children[1].value, bytesmith::Value::UInt(0x0203));
        let rest = &value.children[2];
        assert_eq!(rest.value, bytesmith::Value::Bytes(&[0xaau8, 0xbb]));
        ...
        let tree = packet.field_tree();
        let value = &tree.children[2];
        assert_eq!(value.value, bytesmith::Value::Opaque);
        assert_eq!(value.bit_range, 16..32);
        assert_eq!(
            value.status,
            bytesmith::Status::Error(bytesmith::ParseError::NotEnoughData {{
                expected: 3,
                got: 2,
            }})
        );
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(0x99));
    }}
    ...
    fn field_tree_conditional_present_emits_real_node() {{
        ...
        assert_eq!(options.value, bytesmith::Value::Array);
        ...
        assert_eq!(
            elems,
            [
                bytesmith::Value::UInt(0xaa),
                bytesmith::Value::UInt(0xbb),
                bytesmith::Value::UInt(0xcc),
                bytesmith::Value::UInt(0xdd),
            ]
        );
        assert_eq!(options.children[0].path, "Ipv4WithOptions.options.0");
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(0x11));
    }}
    ...
    fn field_tree_conditional_absent_emits_hidden_absent_node() {{
        ...
        assert_eq!(names, ["version", "ihl", "options", "proto"]);
        let options = &tree.children[2];
        assert_eq!(options.value, bytesmith::Value::Absent);
        assert_eq!(options.status, bytesmith::Status::Ok);
        assert!(options.hidden);
    ...
    fn field_tree_conditional_with_else_picks_branch() {{
        ...
        assert_eq!(tree.children[1].value, bytesmith::Value::UInt(7));
        assert!(!tree.children[1].hidden);
        assert_eq!(tree.children[2].value, bytesmith::Value::Absent);
        assert!(tree.children[2].hidden);
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(9));

        let else_data = [0, 0x12, 0x34, 9];
        let (mut packet, _) = CondElse::parse(&else_data).unwrap();
        let tree = packet.field_tree();
        assert_eq!(tree.children[1].value, bytesmith::Value::Absent);
        assert!(tree.children[1].hidden);
        assert_eq!(tree.children[2].value, bytesmith::Value::UInt(0x1234));
        assert!(!tree.children[2].hidden);
        assert_eq!(tree.children[3].value, bytesmith::Value::UInt(9));
    }}
    ...
    fn field_tree_nested_conditional_struct_ref() {{
        ...
        assert_eq!(first.value, bytesmith::Value::Struct);
        let body = &first.children[1];
        assert_eq!(body.name, "body");
        assert_eq!(body.value, bytesmith::Value::Absent);
        assert!(body.hidden);
    }}
    ...
    fn field_tree_hooks_emit_value_and_error_nodes() {{
        ...
        assert_eq!(tree.children[1].value, bytesmith::Value::UInt(4));
        assert_eq!(tree.children[1].type_name, "u32");
        assert_eq!(tree.children[1].status, bytesmith::Status::Ok);
        assert_eq!(tree.children[2].value, bytesmith::Value::Opaque);
        assert_eq!(tree.children[2].type_name, "String");
        assert_eq!(tree.children[2].bit_range, 24..48);
        assert_eq!(tree.children[2].status, bytesmith::Status::Ok);
    }}
    ...
    fn field_tree_vla_hook_reports_uint_and_range() {{
        ...
        assert_eq!(value.value, bytesmith::Value::UInt(624485));
        assert_eq!(value.bit_range, 8..32);
        assert_eq!(value.status, bytesmith::Status::Ok);
    }}
    ...
    fn field_tree_dns_hook_reports_consumed_range() {{
        ...
        assert_eq!(qname.value, bytesmith::Value::Opaque);
        assert_eq!(qname.bit_range, 16..80);
        assert_eq!(qname.status, bytesmith::Status::Ok);
        let aname = &tree.children[3];
        assert_eq!(aname.bit_range, 96..112);
        assert_eq!(aname.status, bytesmith::Status::Ok);
    }}
    ...
    fn field_tree_union_dynamic_variant() {{
        ...
        assert_eq!(body.value, bytesmith::Value::UnionVariant("Msg"));
        assert_eq!(body.bit_range, 8..40);
        assert_eq!(body.status, bytesmith::Status::Ok);
        assert_eq!(body.children.len(), 1);
        let msg = &body.children[0];
        assert_eq!(msg.name, "Msg");
        assert_eq!(msg.path, "Dispatch.body.Msg");
        assert_eq!(msg.children[0].value, bytesmith::Value::UInt(3));
        let mut inner = &msg.children[1];
        assert_eq!(inner.value, bytesmith::Value::Array);
    ...
    fn field_tree_union_error_arm_surfaces_failed_status() {{
        ...
        let tree = packet.field_tree();
        let body = &tree.children[1];
        assert_eq!(body.value, bytesmith::Value::Opaque);
        assert_eq!(body.type_name, "union");
    ...
    fn field_tree_union_tuple_dispatch() {{
        ...
        assert_eq!(body.value, bytesmith::Value::UnionVariant("Echo"));
        assert_eq!(body.bit_range, 16..48);
        let echo_node = &body.children[0];
        assert_eq!(echo_node.name, "Echo");
        assert_eq!(echo_node.children[0].value, bytesmith::Value::UInt(0x1234));
        assert_eq!(echo_node.children[1].value, bytesmith::Value::UInt(1));

        let raw = [42, 7, 0xff];
        let (mut packet, _) = Icmp::parse(&raw).unwrap();
        let tree = packet.field_tree();
        let body = &tree.children[2];
        assert_eq!(body.value, bytesmith::Value::UnionVariant("Raw"));
    ...
    fn field_tree_union_inside_concat() {{
        ...
        assert_eq!(pair.children[0].value, bytesmith::Value::UInt(0x42));
        let word = &pair.children[1];
        assert_eq!(word.value, bytesmith::Value::UnionVariant("Word"));
        assert_eq!(word.children[0].name, "Word");
        assert_eq!(word.children[0].children[0].value, bytesmith::Value::UInt(0x1234));
        let bytes = &pair.children[2];
        assert_eq!(bytes.value, bytesmith::Value::UnionVariant("Bytes"));
        let bytes_inner = &bytes.children[0];
        assert_eq!(bytes_inner.name, "Bytes");
        assert_eq!(bytes_inner.children[0].value, bytesmith::Value::UInt(2));
    }}
    ...
    fn field_tree_until_greedy_and_struct_arrays() {{
        ...
        assert_eq!(name.children[1].value, bytesmith::Value::UInt(b'i' as u128));
        ...
        assert_eq!(elems, [bytesmith::Value::UInt(0x1234), bytesmith::Value::UInt(0x5678)]);
        ...
        assert_eq!(items.children[1].children[1].value, bytesmith::Value::UInt(0x0506));
    ...
    fn field_tree_reports_element_errors_without_panicking() {{
        ...
        assert_eq!(opts.children[0].value, bytesmith::Value::Struct);
        assert_eq!(opts.children[0].path, "Opts.opts.0");
        assert_eq!(
            opts.children[8].status,
            bytesmith::Status::Error(bytesmith::ParseError::MaxIterationsExceeded {{
                field: "Opts.opts",
                max: 8,
            }})
        );
    ...
    fn struct_without_payload_has_no_handoff() {{
        ...
        let dissect: &mut dyn bytesmith::Dissect = &mut packet;
        assert!(dissect.handoff().is_none());
    }}

    fn parse_for_key(key: u128, payload: &[u8]) -> Option<bytesmith::Handoff<'_>> {{
        match key {{
    ...
    fn struct_level_len_trailing_inside_bound_exposed_and_advances() {{
        ...
        assert_eq!(trailing.value, bytesmith::Value::Bytes(&[0xaa, 0xbb, 0xcc]));
    }}
