...
"#;
    ...
    assert!(
        code.contains("fn data_raw(&mut self) -> ::bytesmith::ParseResult<(MyResult, usize)>"),
        "should have fallible raw helper"
    );
    assert!(
        code.contains("pub fn data(&mut self) -> ::bytesmith::ParseResult<MyResult>"),
