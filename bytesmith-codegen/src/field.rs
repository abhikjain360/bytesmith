...
pub enum Error {
    ...
    MisalignedField { offset: bytesmith::Len, align: usize },
...
impl FieldAccum {
    pub(crate) fn new(field_name: &str) -> Self {
        ...
        Self {
            ...
            len: GeneratedLen::Fixed(bytesmith::Len { byte: 0, bit: 0 }),
...
pub(crate) fn generate<'a>(
    ...
    if let Some(pad) = attrs.pad {
        struct_accum.offset =
            struct_accum.offset.clone() + GeneratedLen::Fixed(bytesmith::Len { byte: pad, bit: 0 });
    }
    ...
    let start_offset = std::mem::replace(
        ...
        GeneratedLen::Fixed(bytesmith::Len { byte: 0, bit: 0 }),
    );
    ...
    let start_offset_body = if has_padding {
        match &start_offset {
            GeneratedLen::Fixed(len) => {
                let byte = len.byte;
                let bit = len.bit;
                quote! { bytesmith::Len { byte: #byte, bit: #bit } }
            }
    ...
    field_accum.offset_getter = match &end_offset {
        GeneratedLen::Fixed(total_len) => {
            ...
            quote! {
                #dead_code
                #vis fn #offset_getter_fn_name(&mut self) -> bytesmith::Len {
                    bytesmith::Len { byte: #total_byte, bit: #total_bit }
                }
            }
        }
        GeneratedLen::Dynamic(total_len) if attrs.cache_len => {
            ...
            struct_accum.cache_field_defs.extend(quote! {
                #cache_ident: Option<bytesmith::Len>,
            });
    ...
    field_accum.offset_getter.extend(quote! {
        #dead_code
        #vis fn #start_offset_getter_fn_name(&mut self) -> bytesmith::Len {
            #start_offset_body
        }
    ...
    let pad_push = if has_padding {
        ...
        struct_accum.functions.extend(quote! {
            #[allow(dead_code)]
            fn #pad_node_fn_name(&mut self) -> Option<::bytesmith::FieldNode<'a>> {
                ...
                if bit_range.start < bit_range.end {
                    Some(
                        ::bytesmith::FieldNode::new(
                                ...
                                "pad",
                                bit_range.clone(),
                                ::bytesmith::Value::bytes(self.data, &bit_range),
                            )
                            .hide(),
                    )
                } else {
    ...
    struct_accum.functions.extend(quote! {
        #[allow(dead_code)]
        fn #present_node_fn_name(&mut self) -> ::bytesmith::FieldNode<'a> {
    ...
    let absent_push = if struct_accum.condition.is_some() {
        ...
        struct_accum.functions.extend(quote! {
            #[allow(dead_code)]
            fn #absent_node_fn_name(&mut self) -> ::bytesmith::FieldNode<'a> {
                let start = self.#start_offset_getter_fn_name().bits();
                ::bytesmith::FieldNode::new(
                        #absent_name,
                        #stub_label,
                        start..start,
                        ::bytesmith::Value::Absent,
                    )
                    .hide()
            }
        });
        ...
        fatal_check.extend(quote! {
            {
                ...
                if !offset.is_byte_aligned() || !offset.byte.is_multiple_of(#align) {
                    return Err(::bytesmith::ParseError::Misaligned {
                        field: #field_path,
                        align: #align,
                        offset,
                    });
    ...
    struct_accum.functions.extend(quote! {
        #[allow(dead_code)]
        fn #fatal_check_fn_name(&mut self) -> Result<(), ::bytesmith::ParseError> {
            #fatal_check
            Ok(())
        }
    });
    ...
    struct_accum.functions.extend(quote! {
        #[allow(dead_code)]
        fn #recoverable_check_fn_name(&mut self) -> Result<(), ::bytesmith::ParseError> {
            #recoverable_check
            Ok(())
        }
    });
...
fn generate_validations<'a>(
    ...
    struct_accum.functions.extend(quote! {
        ...
        fn #validate_fn_name(&mut self) -> Result<(), ::bytesmith::ParseError> {
            #validations
            Ok(())
        }
    });
...
fn generate_vla_hook<'a>(
    ...
    if let Some(len_expr) = &attrs.len {
        ...
        let raw_fn = if let Some(cache_ident) = &value_cache {
            quote! {
                fn #raw_fn_name(&mut self) -> ::bytesmith::ParseResult<(&#return_ty, usize)> {
        ...
        field_accum.field_getter = if attrs.cache_value {
            quote! {
                #dead_code
                #vis fn #field_name(&mut self) -> ::bytesmith::ParseResult<&#return_ty> {
        ...
        field_accum.len = match lowered.const_value {
            Some(byte) => GeneratedLen::Fixed(bytesmith::Len { byte, bit: 0 }),
            ...
            match self.#raw_fn_name() {
                Ok((#bind, consumed)) => {
                    let consumed_end = bit_range
                        ...
                        .min(bit_range.end);
                    let mut node = ::bytesmith::FieldNode::new(
                        #name_str,
                        #type_name,
                        bit_range.clone(),
                        #value,
    ...
    field_accum.field_getter = match &struct_accum.condition {
        Some(gate) => {
            if attrs.cache_value {
                quote! {
                    #dead_code
                    #vis fn #field_name(&mut self) -> Option<::bytesmith::ParseResult<&#return_ty>> {
                        if self.#gate() {
    ...
    field_accum.len = GeneratedLen::Dynamic(quote! {
        match self.#raw_fn_name() {
            Ok((_, consumed)) => bytesmith::Len { byte: consumed, bit: 0 },
    ...
    field_accum.tree_body = quote! {
        match self.#raw_fn_name() {
            Ok((#bind, _)) => ::bytesmith::FieldNode::new(
                #name_str,
                #type_name,
                bit_range.clone(),
                #value,
...
fn hook_value(type_name: &str, value: TokenStream) -> TokenStream {
    match type_name {
        "u8" | "u16" | "u32" | "u64" | "u128" => {
            quote! { ::bytesmith::Value::UInt(u128::from(#value)) }
        }
        "i8" | "i16" | "i32" | "i64" | "i128" => {
            quote! { ::bytesmith::Value::Int(i128::from(#value)) }
        }
        _ => quote! { ::bytesmith::Value::Opaque },
    }
}
...
fn generate_fixed_hook<'a>(
    ...
    field_accum.field_getter = match &struct_accum.condition {
        Some(gate) => {
            ...
            if let Some(cache_ident) = &value_cache {
                quote! {
                    #[allow(clippy::identity_op)]
                    fn #inner_fn_name(&mut self) -> ::bytesmith::ParseResult<&#return_ty> {
                        if self.#cache_ident.is_none() {
                            let value = { #hook_body }?;
                            self.#cache_ident = Some(value);
                        }
    ...
    field_accum.tree_body = quote! {
        match self.#raw_fn_name() {
            Ok(#bind) => ::bytesmith::FieldNode::new(
                #name_str,
                #type_name,
                bit_range.clone(),
                #value,
