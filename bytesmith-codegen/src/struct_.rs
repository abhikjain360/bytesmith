...
pub(crate) fn generate_struct<'a>(
    ...
    let dissect_body = if dissect_stmts.is_empty() {
        quote! {
            let _ = Self { data };
            let children: ::std::vec::Vec<::bytesmith::FieldNode<'a>> = ::std::vec::Vec::new();
            let mut root = ::bytesmith::FieldNode::new(
                    #struct_name_str,
                    #struct_name_str,
                    0usize..0usize,
    ...
    let dissect_impl = quote! {
        impl<'a> ::bytesmith::Dissect<'a> for #name<'a> {
        ...
        quote! {
            pub fn parse(data: &'a [u8]) -> Result<(Self, &'a [u8]), ::bytesmith::ParseError> {
                ...
                if data.len() < len.byte {
                    return Err(::bytesmith::ParseError::NotEnoughData { expected: len.byte, got: data.len() });
                }
                Ok((me, &data[len.byte..]))
            }
        }
...
fn apply_struct_len(accum: &mut StructAccum<'_>) -> Result<Option<syn::Ident>, Error> {
    ...
    accum.functions.extend(quote! {
        #[allow(dead_code)]
        fn #struct_len_fn_name(&mut self) -> bytesmith::Len {
...
fn generate_conditional<'a>(
    ...
    accum.functions.extend(quote! {
        fn #end_offset_fn_name(&mut self) -> bytesmith::Len {
