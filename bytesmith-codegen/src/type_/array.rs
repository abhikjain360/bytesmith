...
pub(crate) fn generate<'a>(
                    ...
                    GeneratedLen::Dynamic(_) => {
                        ...
                        ElemGen {
                            len: GeneratedLen::Dynamic(quote! {{
                                ...
                                ::bytesmith::Len { byte: #data.len().saturating_sub(start), bit: 0 }
                            }}),
                            return_ty,
                            iterator_fields: quote! {
                                idx: usize,
                            ...
                            next_fn_body: quote! {
                                ...
                                if self.idx == self.max {
                                    self.data = &[];
                                    return Some(Err(::bytesmith::ParseError::MaxIterationsExceeded {
                                        field: #field_path,
                                        max: self.max,
                                    }));
                                }
    ...
    struct_accum.other_entities.extend(quote! {
        ...
        impl<'a> ::std::iter::Iterator for #iterator_name<'a> {
            type Item = ::bytesmith::ParseResult<#elem_return_ty>;
        ...
        && let Some(check_count) = &check_count
    {
        ...
        accum.helper_fns.extend(quote! {
            fn #validate_fn_name(&mut self) -> Result<(), ::bytesmith::ParseError> {
                ...
                let max = #max;
                if count > max {
                    return Err(::bytesmith::ParseError::MaxIterationsExceeded {
                        field: #field_path,
                        max,
                    });
                }
    ...
    Ok(GeneratedTypeInfo {
        len: field_len,
        field_getter_body,
        return_ty: quote! { ::bytesmith::ParseResult<#iterator_name<'a>> },
...
fn tree_node(
    ...
    let elem_loop = match &array_type.elem_ty.kind {
        ...
        ast::ArrayElemTypeKind::BitField(width) => {
            ...
            let value_ctor = quote! { ::bytesmith::Value::UInt(u128::from(value)) };
            elem_value_loop(
                getter,
                quote! { #elem_bits },
                &elem_label,
    ...
    Ok(quote! {
        {
            ...
            #elem_loop
            ::bytesmith::FieldNode::new(#name_str, #type_label, bit_range.clone(), ::bytesmith::Value::Array)
    ...
    quote! {
        if let Ok(iter) = self.#getter() {
            ...
            for (i, elem) in iter.enumerate() {
                ...
                match elem {
                    Ok(value) => elem_nodes.push(::bytesmith::FieldNode::new(
                        i.to_string(),
                        #elem_label,
                        start..end,
                        #value_ctor,
    ...
    match static_count {
        ...
        None => match elem_len {
            GeneratedLen::Fixed(Len { byte, bit }) => GeneratedLen::Dynamic(quote! {
                ::bytesmith::Len { byte: #byte, bit: #bit } * (#count)
            }),
