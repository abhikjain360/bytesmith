...
pub(crate) fn generate<'a>(
            ...
            ast::UnionBody::NamedInline(inline_struct) => {
                ...
                let variant_value_len = match &generated.len {
                    GeneratedLen::Fixed(len) => {
                        ...
                        let bit = len.bit;
                        quote! {
                            #enum_name::#variant_ident(_) => ::bytesmith::Len { byte: #byte, bit: #bit },
                        }
                    }
                    GeneratedLen::Dynamic(_) => {
                        let last_offset_getter = generated
                    ...
                    tree_arms.extend(quote! {
                        Ok(#enum_name::#variant_ident(value)) => {
                            ...
                            ::bytesmith::FieldNode::new(
                                    ...
                                    "union",
                                    bit_range.clone(),
                                    ::bytesmith::Value::UnionVariant(#variant_name),
                                )
    ...
    if attrs.cache_value {
        ...
        let cached_result_ty = if has_error_arms {
            ...
            quote! { ::bytesmith::ParseResult<(#enum_name<'a>, &'a [u8])> }
        };
        let cached_return_ty = if has_error_arms {
            quote! { ::std::result::Result<(&mut #enum_name<'a>, &'a [u8]), Error> }
        } else {
            quote! { ::bytesmith::ParseResult<(&mut #enum_name<'a>, &'a [u8])> }
        };
        ...
        accum.helper_fns.extend(quote! {
            fn #check_fn_name(&mut self) -> Result<(), ::bytesmith::ParseError> {
                #check_body
            }
        });
    } else {
    ...
    let return_ty = match (has_error_arms, attrs.cache_value) {
        ...
        (false, true) => quote! { ::bytesmith::ParseResult<&mut #enum_name<'a>> },
        (false, false) => quote! { ::bytesmith::ParseResult<#enum_name<'a>> },
    };
    ...
    let err_arm = if has_error_arms {
        quote! {
            Err(Error::Parse(error)) => ::bytesmith::FieldNode::new(
                    #field_name_str,
                    "union",
                    bit_range.clone(),
            ...
            Err(error) => ::bytesmith::FieldNode::new(
                ...
                .with_status(::bytesmith::Status::Failed(error.variant_name())),
        }
    } else {
        quote! {
            Err(error) => ::bytesmith::FieldNode::new(
