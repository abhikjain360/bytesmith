...
fn emit_variant_writer(name: &str, layout: &VariantLayout) -> TokenStream {
    ...
    quote! {
        ...
        impl<'a> #writer_name<'a> {
            ...
            pub fn write_into(data: &'a mut [u8], content: &#content_name) -> ::bytesmith::WriteResult<usize> {
                #region_locals
...
fn emit_dynamic_tail(name: &str, fields: &[WriterField], tail: &DynamicTail) -> TokenStream {
    ...
    quote! {
        ...
        impl<'a> #writer_name<'a> {
            ...
            pub fn new(data: &'a mut [u8], lens: #lens_name) -> ::bytesmith::WriteResult<Self> {
                let need = Self::encoded_len(&lens);
                if data.len() < need {
                    return Err(::bytesmith::WriteError::NotEnoughSpace {
                        expected: need,
...
fn emit_union(name: &str, layout: &UnionLayout) -> TokenStream {
    ...
    quote! {
        ...
        impl<'a> #writer_name<'a> {
            ...
            pub fn write_into(data: &'a mut [u8], content: &#content_name #body_lt) -> ::bytesmith::WriteResult<usize> {
                let need = Self::encoded_len(content);
                if data.len() < need {
                    return Err(::bytesmith::WriteError::NotEnoughSpace {
                        expected: need,
