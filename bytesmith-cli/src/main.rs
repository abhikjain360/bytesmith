...
#[derive(Parser)]
#[command(name = "bytesmith-cli", version, about = "Compile bytesmith DSL specs into Rust source")]
struct Cli {
    #[arg(required = true, value_name = "FILE", help = "`.bsm` spec files to compile (use `-` for stdin)")]
    inputs: Vec<PathBuf>,
...
fn main() -> ExitCode {
    ...
    for input in &cli.inputs {
        ...
        let ast = match bytesmith_dsl_parse::parse_str(&src) {
            Ok(ast) => ast,
            Err(report) => {
                eprint!("{report}");
                return ExitCode::FAILURE;
        ...
        let code = if cli.no_writers {
            bytesmith_codegen::CodeGen::generate(&ast)
        } else {
            bytesmith_codegen::CodeGen::generate_writers(&ast)
        };
        let code = match code {
            Ok(code) => code,
            Err(e) => {
