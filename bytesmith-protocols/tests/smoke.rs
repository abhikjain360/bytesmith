...
fn ethernet_parses_arp_frame() {
    ...
    use bytesmith_protocols::ethernet::EthernetII;
...
#[test]
fn vlan_parses() {
    use bytesmith_protocols::vlan::Vlan;
...
#[test]
fn arp_parses() {
    use bytesmith_protocols::arp::Arp;
    let packet = vec![
...
#[test]
fn ipv4_parses() {
    use bytesmith_protocols::ip::Ipv4;
    let packet = vec![
...
#[test]
fn ipv6_parses() {
    use bytesmith_protocols::ip::Ipv6;
...
fn icmpv4_echo_parses() {
    use bytesmith_protocols::icmp::{Icmpv4, Icmpv4_body};
...
fn icmpv6_echo_parses() {
    use bytesmith_protocols::icmpv6::{Icmpv6, Icmpv6_body};
...
#[test]
fn udp_parses() {
    use bytesmith_protocols::udp::Udp;
    let packet = vec![
...
#[test]
fn tcp_parses() {
    use bytesmith_protocols::tcp::Tcp;
    let packet = vec![
...
fn dns_parses_with_compression() {
    use bytesmith_protocols::dns::Dns;
    let packet = vec![
...
fn tls_record_parses() {
    use bytesmith_protocols::tls::TlsRecord;
...
fn dhcp_discover_parses() {
    use bytesmith_protocols::dhcp::Dhcp;
...
fn sctp_single_chunk_parses() {
    use bytesmith_protocols::sctp::Sctp;
    let packet = vec![
...
fn bgp_keepalive_parses() {
    use bytesmith_protocols::bgp::{Bgp, Bgp_body};
...
fn mqtt_v3_connect_parses() {
    use bytesmith::ParseResult;
    use bytesmith_protocols::mqtt_v3::{MqttPacket, MqttPacket_body};
    let packet = vec![
...
fn mqtt_v5_connack_properties_sized_by_hook_varint() {
    use bytesmith::ParseResult;
    use bytesmith_protocols::mqtt_v5::{MqttPacket, MqttPacket_body};
