...
use bytesmith_codegen::CodeGen;

const SOURCE: &str = "bytesmith";

/// Run the parser, then codegen, over a `.bsm` source string and surface any
/// failure as LSP diagnostics. This is the whole wrapper: it reuses the error
/// reporting already produced by `bytesmith-dsl-parse` and `bytesmith-codegen`.
pub fn compute(text: &str) -> Vec<Diagnostic> {
    let (defs, errors) = bytesmith_dsl_parse::parse_str_recover(text);
    if !errors.is_empty() {
        return errors
            .into_iter()
